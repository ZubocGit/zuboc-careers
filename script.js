...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const animatedElements = document.querySelectorAll(
    ".value-item, .position-card, .benefit-item, .process-step"
  );
  ...
  newsletterForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(
          `Subscription failed: ${response.status} ${response.statusText} - ${errorText}`
        );
      }
    } catch (error) {
      console.error("Newsletter subscription error:", error);
      alert(
        `There was an error subscribing to our newsletter: ${error.message}. Please try again.`
      );
      button.textContent = originalText;
      button.disabled = false;
    }
  ...
  validateForm() {
    ...
    if (!cvFile) {
      ...
      if (!isValidType) {
        this.setFieldError(
          cvInput,
          "Please upload a valid file (PDF, DOC, DOCX, or TXT)"
        );
        isValid = false;
      } else if (!isValidSize) {
  ...
  async handleSubmit(e) {
    ...
    try {
      ...
      if (this.roleSelect && this.roleSelect.value) {
        ...
        formData.set(
          "role",
          this.roleSelect.options[this.roleSelect.selectedIndex].textContent
        );
      }

      // Submit to webhook
      const response = await fetch(
        "https://n8n.srv1052463.hstgr.cloud/webhook/candidate-application",
        {
          method: "POST",
          body: formData,
        }
      ...
      if (response.ok) {
        ...
        throw new Error(
          `Submission failed (${response.status} ${response.statusText}) ${respText}`
        );
      }
    } catch (error) {
...
document
  .querySelectorAll(".position-card, .benefit-item, .value-item")
  .forEach((card) => {
